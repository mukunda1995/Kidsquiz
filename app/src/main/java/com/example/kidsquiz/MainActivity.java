...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //To add all animals and names into the list

        for(int i = 0; i < new Database().answers.length; i++) {
            list.add(new ItemAnimal(new Database().answers[i],new Database().animals[i]));
        }

        //To shuffle the animals
        ...
        b_answer1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(b_answer1.getText().toString().equalsIgnoreCase(list.get(turn-1).getName())){
                    ...
                    //To check the question number
                    if(turn< list.size()){
                        turn++;
                        newQuestion(turn);

                    }else{
                        Toast.makeText(MainActivity.this,"Game completed",Toast.LENGTH_SHORT).show();
                        finish();
    ...
    private void newQuestion(int number){
        //To set the animal image to the screen
        iv_animal.setImageResource(list.get (number - 1).getImage());

        //To place the correct answer on which button
